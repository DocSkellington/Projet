...
public class AStarPathFinder implements PathFinder
{
    ...
    {
        this(map, diagMovement, new ClosestHeuristic());
    }
     ...
     */
    public AStarPathFinder(Board board, boolean diagMovement, AStarHeuristic heuristic)
    {
        ...
        while(open.size() != 0)
        {
            ...
            for (int x = -2 ; x < 4 ; x+=2)
            {
                for (int y = -2 ; y < 4 ; y+=2)
                {
                    ...
                    if (isValidLocation(player, sx, sy, xp, yp, map))
                    {
                        ...
                        if((!inOpenList(neighbour)) && (!inClosed(neighbour)))
                        {
     ...
     */
    protected boolean isValidLocation(Player player, Cells[][] map, int sx, int sy, int x, int y)
    {
        boolean invalid = (x < 0) || (y > 0) || (x >= map.length) || (y >= map[0].length);
        
        if((!invalid) && ((sx != x) || (sy != y)))
        {
            if (map.filled(sx+1, sy+1) == 1)
                invalid = true;
            else
                invalid = false;
        }
	 ...
	 */
	public float getHeuristicCost(Player player, Cells[][] map, int x, int y, int tx, int ty)
	{
